...
import { useRouter, useSearchParams } from 'next/navigation'; // Now imported here
...
export default function AiChatClient() { // Renamed from AiChatPage to AiChatClient
  ...
  useEffect(() => {
    const auth = getAuth();
    if (!user && !authLoading && !anonymousUser) {
  ...
  }, [user, authLoading, anonymousUser]);
  ...
  }, [activeUser, searchParams, currentChatSessionId, router, chatSessions]);
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where("chatSessionId", "==", currentChatSessionId),
    ...
    return () => unsubscribe();
  }, [activeUser, authLoading, userChatCollectionPath, currentChatSessionId]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!sessionDocSnapshot.exists() || messages.length === 0) {
      ...
      formData.append('chat_session_id', sessionToUse);
  ...
  return (
    <section className="flex flex-1 h-[calc(100vh-80px)] relative">
