...
// This component contains the actual client-side logic for the pricing page.
"use client";
...
export default function PricingPageClient() { // Renamed to PricingPageClient
  const { user, loading: authLoading } = useAuth();
  const router = useRouter();
  const [loadingPayment, setLoadingPayment] = useState<string | null>(null);
  const [paymentError, setPaymentError] = useState<string | null>(null);

  // ✅ CORRECT: Call usePaystackPayment at the top level of this client-only component
  // This hook will now only be executed in the browser environment.
  ...
  const handleSubscribeClick = async (planId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to initiate payment with backend.');
      }
      ...
      if (authorizationUrl) {
        // Now, initializePayment() is guaranteed to be callable in a client environment
        const config = {
        ...
        {plans.map((plan) => (
          <div
            ...
            className={`
              ...
              ${user?.subscriptionPlan === plan.id ? 'bg-indigo-50' : ''}
            `}
          >
            {plan.isPopular && (
